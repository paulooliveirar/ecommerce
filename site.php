...
$app->post('/register', function(){
	...
	if(User::checkLoginExists($_POST['email']) === true){
		User::setErrorRegister("Você já possui cadastro no nosso sistema.");
		header("Location: /login");
		exit;		
	}
...
$app->get("/boleto/:idorder", function($idorder){
	...
	$dadosboleto["endereco1"] = $order->getdesaddress() . " " . $order->getdesdistrict() . ", " . $order->getdesnumber() . " - CEP:" . $order->getdeszipcode();
...
$app->post("/profile/change-password", function(){
	...
	if($_POST['current_pass'] === $_POST['new_pass']){
		User::setError("Sua senha deve ser diferente da atual.");
		header("Location: /profile/change-password");
		exit;
	}

	if($_POST['new_pass'] != $_POST['new_pass_confirm']){
		User::setError("A Confirmação da nova senha digitada está incorreta.");
		header("Location: /profile/change-password");
		exit;
	}	
