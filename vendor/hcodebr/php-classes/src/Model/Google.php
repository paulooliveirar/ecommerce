...
class Google {
	...
	public static function getID(){
		...
		return $sql->select("SELECT * FROM tb_methodsshipping ORDER BY idshipping");
	}
	...
	public static function listMethodOn(){
		...
		return $sql->select("SELECT * FROM tb_methodsshipping WHERE inactive = 1");
	}

	public function updateStatus($idshipping, $inactive){
		$sql = new Sql();
		...
		$sql->query("UPDATE tb_methodsshipping SET inactive = :inactive WHERE idshipping = :idshipping", array(
			":inactive"=>$inactive,
			":idshipping"=>$idshipping
		));
	}

	public function updateStatusSandbox($idshipping, $sandbox){
		$sql = new Sql();
		...
		$sql->query("UPDATE tb_methodsshipping SET sandbox = :sandbox WHERE idshipping = :idshipping", array(
			":sandbox"=>$sandbox,
			":idshipping"=>$idshipping
		));
	}
	...
	public function get($idshipping){
		$sql = new Sql();

		$results = $sql->select("SELECT * FROM tb_methodsshipping WHERE idshipping = :idshipping", array(
			":idshipping"=>$idshipping
		));
