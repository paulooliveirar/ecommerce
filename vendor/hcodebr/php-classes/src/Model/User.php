...
class User extends Model {
	...
	const SUCCESS = "UserSuccessRegister";
	...
	public static function login($login, $password){
		...
		if(password_verify($password, $data["despassword"]) === true){
			...
			User::createLog("Usuário fez login");
	...
	public static function createLog($msg){
		...
		if(isset($_SESSION[User::SESSION]) != ''){
			$sessionid = $_SESSION[User::SESSION]['deslogin'];
		...
		$results = $sql->select("CALL sp_userslog_save(:iduser, :deslog, :desip, :desuseragent, :dessessionid, :desurl)", [
			":iduser"=>$iduser,
			":deslog"=>utf8_decode($msg),
			":desip"=>$_SERVER["REMOTE_ADDR"],
			":desuseragent"=>$_SERVER['HTTP_USER_AGENT'],
	...
	public static function verifyLogin($inadmin = true){
		if(!User::checkLogin($inadmin)){
			if($inadmin){
				User::createLog("Verificar Login de Admin");
				header("Location: /admin/login");
			}
			else{
				User::createLog("Verificar Login de Comprador");				
	...
	public static function logout(){
		User::createLog("Usuário fez logout");
