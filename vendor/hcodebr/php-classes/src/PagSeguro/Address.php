...
class Address{
	...
	public function __construct(string $street, int $number, string $complement, string $district, string $postalCode, string $city, string $state, string $country){

		if(!$street){
			throw new Exception("Informe o lougradouro do endereço.");
		}
		if(!$number){
		...
		$this->street = $street;
		$this->number = $number;
		if($complement == ""){$complement = "nada consta";}
